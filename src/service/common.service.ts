...
export class CommonService {
  ...
  getDistricts(): Observable<any> {
    return this.httpClient
      .get<any>(
        `http://localhost:8080/empower_andhra/districts?stateId=1`
      )
      .pipe(
        map((Districts) => {
          return Districts;
  ...
  getVillages(mandalCode: string): Observable<any> {
    return this.httpClient
      .get<any>(
        `http://localhost:8080/empower_andhra/villages?mandalId=${mandalCode}`
      )
      .pipe(
        map((Villages) => {
          return Villages;
  ...
  getProjectCategories(): Observable<any> {
    return this.httpClient
      .get<any>(
        `http://localhost:8080/empower_andhra/project-categories`
      )
      .pipe(
        map((projects) => {
          return projects;
  ...
  getProjects(): Observable<any> {
    return this.httpClient
      .get<any>(`http://localhost:8080/empower_andhra/projects`)
      .pipe(
        map((Projects) => {
          return Projects;
        }),
